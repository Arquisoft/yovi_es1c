...
|Operations that affect multiple data domains (e.g., user deletion, which must remove records from `authdb` (PostgreSQL), `users.db`, and `gamedb` (PostgreSQL)) currently have no distributed transaction mechanism. This is accepted as a technical debt for the academic scope of the project and should be addressed with compensating transactions or a saga pattern in a production-grade system.
|===