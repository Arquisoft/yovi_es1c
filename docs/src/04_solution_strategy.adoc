...
• **Security**: Isolates sensitive credential data (password hashes, tokens) in a dedicated database (`authdb` (PostgreSQL))
...
a| • **Single Responsibility**: Manages all game-related persistence: matches, moves, and statistics (`gamedb` (PostgreSQL))
...
| **Databases (PostgreSQL x2 + SQLite x1)**
a| • **Fault Isolation**: Three independent databases (`authdb` (PostgreSQL), `users.db`, `gamedb` (PostgreSQL)) ensure that a failure or overload in one does not affect the others
...
note right of auth
  **Auth Service Responsibilities:**
  ...
  • Credentials persistence (authdb)
end note
...
* **Auth Service** is exclusively responsible for authentication, registration, and JWT management, backed by `authdb` (PostgreSQL)
* **Users Service** manages user profile data, backed by `users.db`
* **Game Service** manages all game-related persistence (matches, moves, statistics), backed by `gamedb` (PostgreSQL)
...
* **CredentialsRepository**: Abstracts access to `authdb` (PostgreSQL)
...
* **MatchRepository / StatsRepository**: Abstract access to `gamedb` (PostgreSQL)
...
• PostgreSQL for auth/game transactional workloads and SQLite for users profile storage
...
• `authdb` (PostgreSQL) isolated from `users.db` and `gamedb` (PostgreSQL)
|===
...
| Three independent databases (authdb, users.db, gamedb) ensure that overload or failure in one domain does not affect the others
|===
