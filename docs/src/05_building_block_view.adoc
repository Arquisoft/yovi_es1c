...
| **RankingService**
a| • Computes ELO rating changes after each ranked match (BOT and ONLINE; LOCAL_2P skipped)
• Adapts the K-factor by experience (K=40 under 30 games, K=20 afterwards)
• Maps bot difficulty to a fixed opponent rating and resolves the real opponent rating for online matches
• Exposes the leaderboard and per-user ranking queries consumed by `GameController`
