...
| **AuthController**
| • Handles HTTP requests for `/auth/register` and `/auth/login`
• Validates user credentials
• Returns JWT tokens or session cookies

| **MatchesController**
| • Manages match creation (`POST /matches`)
• Retrieves match history (`GET /matches`)
• Requires authentication

| **StatsController**
| • Provides player statistics endpoint (`GET /stats`)
• Aggregates data from UsersService

| **BotApiController**
| • Exposes bot-specific API (`POST /bot/play`)
...
| **UsersClient**
| • HTTP client wrapping UsersService API
• Handles user authentication, match persistence, statistics retrieval

| **GameEngineClient**
| • HTTP client wrapping Game Server API
...
| **EngineService**
| • REST API entry point (`POST /ybot/choose/{botId}`)
...
| **UsersApiController**
| • Exposes internal REST API for Web App Server
...
| **MatchService**
| • Match creation and state persistence
...
| **MatchRepository**
| • Database access layer for `matches` table
• Stores serialized YEN board states

| **StatsRepository**
| • Database access layer for aggregated statistics
