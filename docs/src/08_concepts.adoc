...
Only authenticated users can access certain features, such as creating matches or viewing their profile. The Users Service handles all authentication using JWT tokens.
...
• **Data Protection:**
We only store the minimum amount of user information needed for the system to work. The database is accessible only through the Users Service, ensuring data access control.
...
• **Users Service (users/):** Node.js + Express + TypeScript service that manages user registration, authentication, match persistence, and statistics. It is the single point of access to the SQLite database.
...
The application relies on a SQL database (SQLite - relational database) that will
manage the data of the application related to user accounts, match history, and statistics.

The database is accessed exclusively by the Users Service. This design decision ensures:

• **Data Consistency:** Only one service manages database transactions
• **Security:** Other services cannot directly access or modify the database
• **Separation of Concerns:** Game Server remains stateless and focused on game logic

Game sessions are computed in memory by the Rust Game Server, which is stateless. When a match needs to be persisted (for history or resuming later), the Users Service stores the game state in YEN notation in the SQLite database.
