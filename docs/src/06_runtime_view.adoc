...
Nginx forwards the request to the appropriate service, which verifies the token internally or delegates to Auth Service.
...
alt invalid YEN or game finished
  ...
  Nginx -> GameService: Forward to port 3002
  GameService -> GameService: Verify token + matchId ownership
