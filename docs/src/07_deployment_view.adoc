...
node "Cloud Virtual Machine" as vm {
  ...
  node "Docker Network (internal)" as dockernet {
    ...
    database "authdb\n(PostgreSQL Volume)" as authdb
    database "users.db\n(Host Volume)" as usersdb
    database "gamedb\n(PostgreSQL Volume)" as gamedb
  }
}
...
* **Fault Isolation:** Each service owns its own data store. An overload or failure in `gamedb` (PostgreSQL) does not affect `authdb` (PostgreSQL) or `users.db`. Similarly, a Redis failure only affects online matchmaking and active sessions, while AI matches and authentication remain fully operational.
...
| Deployed as static files (HTML, CSS, JS bundle) served by Nginx over HTTPS (`443`, with `80` redirect). The React application runs entirely in the user's browser. Built with Vite and bundled for production.

| **Nginx (API Gateway)**
| Deployed in a Docker container inside the VM. It works as a Reverse Proxy and API Gateway. It is the only public ingress layer (Port 443 with TLS, Port 80 redirect) to route all incoming traffic. Handles CORS, rate limiting, WebSocket proxying for Socket.IO, and serves the React frontend static files. Routes API requests to backend services.

| **Auth Service (Node.js + Express + TypeScript)**
| Deployed in a Docker container inside the VM (Port 3001, internal only). Exclusively manages user credentials, registration, authentication, and JWT token lifecycle. No other service has access to `authdb` (PostgreSQL).
...
| Deployed in a Docker container inside the VM (Port 3002, internal only). Exclusively manages match persistence, move history, player statistics, online matchmaking, and real-time sessions via Socket.IO. No other service has access to `gamedb` (PostgreSQL) or the Redis matchmaking/session data.
...
This level shows the internal Docker network rules. The **Nginx API Gateway** completely isolates the microservices from the public internet. External users can never reach the backend services directly. All requests must go through Nginx (HTTPS on `443`; `80` only redirects).
...
1. User opens `https://yourapp.com` in browser
...
6. Auth Service validates credentials against `authdb` (PostgreSQL) and returns JWT token
...
9. Game Service persists the new match in `gamedb` (PostgreSQL) and returns the match ID
...
14. Nginx routes to Game Service (port 3002), which stores the move in `gamedb` (PostgreSQL)
...
3. Match assignment is received via Socket.IO event `matchmaking:matched` (REST polling endpoint kept as fallback)
4. React joins the session with `match:join` and receives `session:state` updates from Game Service
...
7. When the game ends, the session is removed from Redis and the result is persisted in `gamedb` (PostgreSQL)
...
* `authdb` (PostgreSQL) is mounted exclusively inside the Auth Service container. Only Auth Service has read/write permissions. Stores credentials and JWT metadata.
...
* `gamedb` (PostgreSQL) is mounted exclusively inside the Game Service container. Only Game Service has read/write permissions. Stores matches, moves, and statistics.
...
All backend services (Auth Service, Users Service, Game Service, Game Server, Redis) run on an internal Docker network. They cannot be accessed directly from outside the VM. Nginx exposes ports 80 and 443 (80 redirects to HTTPS), while Prometheus (9090) and Grafana (9091) are also published for observability. This architecture ensures that:
