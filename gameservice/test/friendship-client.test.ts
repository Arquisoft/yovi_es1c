...
describe('FriendshipClient', () => {
  ...
  it('requires USERS_SERVICE_URL when no explicit base URL is supplied', () => {
    expect(() => new FriendshipClient()).toThrow('USERS_SERVICE_URL must be configured');
  });

  it('uses USERS_SERVICE_URL from the environment', async () => {
    process.env.USERS_SERVICE_URL = 'http://users:3000/api/users/';
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(
      new Response(JSON.stringify({ friend: true, user: { id: 2, userId: 2, username: 'bea' } }), { status: 200 }),
    );

    const client = new FriendshipClient();
